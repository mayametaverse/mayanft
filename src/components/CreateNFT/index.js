...
import { BiMaya } from "react-icons/bi";
    ...
    return (
      <>
              ...
              <CreateNFTColumn>
                <Form
                  onSubmit={(event) => {
                    ...
                    const Maya_name = this.MayaNameInput.value;
                    this.props.createMaya(price, Maya_name);
                  }}
                >
                  <FormHeading>Create Maya NFT</FormHeading>
                  <BiMaya />
                  <FormInput
                    id="MayaNameInput"
                    type="text"
                    ref={(input) => {
                      this.MayaNameInput = input;
                    }}
                    placeholder="Enter Maya name"
                    required
                  />
                  <FaEthereum />
                  <FormInput
        ...
        <UniverseContainer id="collection">
          ...
          <UniverseWrapper>
            {this.props.Mayas.map((Maya, key) => (
              <UniverseCard key={key}>
                <UniverseIcon
                  src={`https://ipfs.infura.io/ipfs/${Maya.hash}`}
                  width="65%"
                />
                {/* H2 is the Maya_name */}
                <UniverseOwner>{Maya.owner}</UniverseOwner>
                <UniverseH2>{Maya.Maya_name}</UniverseH2>
                ...
                >
                  <FaEthereum /> <span>{Maya.price}</span>
                </Button>
              </UniverseCard>
            ))}
          </UniverseWrapper>
